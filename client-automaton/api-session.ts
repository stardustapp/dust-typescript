import { SkylinkClientDevice, SkylinkReversalExtension, ChannelExtension, InlineChannelCarrier, FolderEntry, StringEntry } from '../skylink/src/mod.ts';
...
async function launchUsingAppToken(apiDevice: SkylinkClientDevice, userId: string, tokenSecret: string) {
  ...
  try {
    const output = await launchEntry.invoke(new FolderEntry('', [
      new StringEntry('User ID', userId),
      new StringEntry('Token', tokenSecret),
    ]));
...
export class ApiSession {
  constructor(
    public apiDevice: SkylinkClientDevice,
    public wsDevice: SkylinkClientDevice,
    public sessionId: string,
  ) {
    // this.closedDevice = new Promise(resolve => this.markClosedDevice = resolve);
  }

  static async findFromEnvironment(env: Record<string,string>) {

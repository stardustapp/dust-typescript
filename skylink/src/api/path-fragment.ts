...
type PathOrigins = string[] | string | PathFragment;

export class PathFragment {
  constructor(isAbsolute: boolean, parts: string[] = []) {
  ...
  static from(thing: PathOrigins, isAbsolute=true) {
    ...
    if (thing.constructor === PathFragment) {
      return thing.clone();
    }
    throw new Error(
  ...
  static parseUri(uri: string) {
    ...
    return {
      scheme, host, hostname, port, query, fragment,
    ...
    if (query && query.startsWith('?')) {
      for (const part of query.slice(1).split('&')) {
        if (part.includes('=')) {
          ...
          params.append('', decodeURIComponent(part));
        }
      }
    }
    return params;
  ...
  slice(start?: number, end?: number) {
    return new PathFragment(this.isAbsolute, this.parts.slice(start, end));
  }

  startsWith(other: PathOrigins) {

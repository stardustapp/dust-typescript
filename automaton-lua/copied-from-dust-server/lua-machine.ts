...
import { FolderEntry,StringEntry,DeviceEntry, Entry, Environment, BlobEntry } from '../../skylink/src/mod.ts';
import { SkyDevice } from "../../skylink/src/types.ts";

import * as api from './lua-api.ts';
import { LUA_API } from './lua-api.ts';
import { CallTrace, TraceContext } from './tracing.ts';

export class LuaContext implements api.LuaContext {
  constructor(
    public lua: fengari.lua.lua_State,
    public rootDevice: Environment,
  ) {}

  compileLuaToStack(T: CallTrace, sourceText: string) {
    T.startStep({name: 'Lua loadstring', bytes: sourceText.length.toString()});
  ...
  readLuaEntry(T: CallTrace, index: number) {
    const L = this.lua;
    ...
    case lua.LUA_TUSERDATA:
      ...
      const raw = lauxlib.luaL_checkudata(L, index, "stardust/root") as {root: Environment};
      return new DeviceEntry("context", raw.root);
  ...
  pushDeviceReference(T: CallTrace, device: SkyDevice) {
    const L = this.lua;
  ...
  pushLiteralEntry(T: CallTrace, entry: Entry | null) {
    const L = this.lua;
    if (entry == null) {
      lua.lua_pushnil(L);
      return;
  ...
  pushLuaTable(T: CallTrace, folder: FolderEntry) {
    const L = this.lua;
    lua.lua_newtable(L);
  ...
  resolveLuaPath(T: CallTrace) {
    ...
    if (lua.lua_isuserdata(L, 1)) {
      const raw = lauxlib.luaL_checkudata(L, 1, "stardust/root") as {root: Environment};
      device = raw.root;
      lua.lua_remove(L, 1);
...
export class LuaMachine extends LuaContext {
  name: string;
  nextThreadNum: number;
  threads: Map<number, LuaThread>;
  luaThreads: Map<fengari.lua.lua_State, LuaThread>;

  constructor(rootDevice: Environment) {
    ...
    for (const callName in LUA_API) {
      ...
      const impl = (L: fengari.lua.lua_State) => {
        const thread = this.luaThreads.get(L)!;
        const argCount = lua.lua_gettop(L);
        const T = thread.traceCtx.newTrace({name: callName, callName, argCount: argCount});
        thread.T = T; // TODO
        ...
        return lua.lua_yield(L, argCount+1);
      };
...
export class LuaThread extends LuaContext {
  ...
  status: string;
  T?: CallTrace;
  luaEnv?: (L: fengari.lua.lua_State) => void;
  runnable?: (L: fengari.lua.lua_State) => void;
  running?: boolean;
  sourceText?: string;

  constructor(machine: LuaMachine, number: number) {
